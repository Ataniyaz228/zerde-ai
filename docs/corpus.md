...
### Исходники корпуса (PDF законов)

Скачанные с adilet.zan.kz PDF-исходники законов лежат в **`data/corpus_sources/`**
(папка на закон + `manifest.json` с путями и версиями). Это бинарные данные,
а не документация — каталог в `.gitignore`. `scripts/update_corpus.py` читает
`data/corpus_sources/manifest.json`. (Ранее эти папки лежали в `docs/`; перенесены,
чтобы `docs/` содержал только проектную документацию `*.md`.)
