"""S2.7 — поиск внутренних противоречий в самом документе (без LLM, только regex).

Ловим два класса: коллизии сроков (одна статья называет разные числа для одного
и того же срока) и хронологические аномалии (дата вступления в силу раньше даты
принятия). Возвращаем найденное как list[DocumentClaim].
"""
    ...
    for eff_year, eff_month, eff_day, eff_text, eff_pos in effective_dates:
        for adop_year, adop_month, adop_day, adop_text, adop_pos in adoption_dates:
            ...
            if eff_tuple < adop_tuple:
                # Сравниваем только пары дат, находящиеся рядом в тексте (в пределах 500 символов)
